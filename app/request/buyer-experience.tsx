...
export function BuyerExperience({ home = false, initialProduct = '', restoreDraft = false, children }: { home?: boolean; initialProduct?: string; restoreDraft?: boolean; children?: ReactNode }) {
  ...
  const [draft, setDraft] = useState<Draft>(() => ({ ...emptyDraft(), ...(initialProduct ? { mode: 'product' as const, query: initialProduct } : {}) }));
  ...
  async function interpret(event: FormEvent) {
    ...
    if (home && saved) { router.push('/request?draft=1'); return; }
    setDraft(next); setRunId(nextRun); setNotice(nextNotice); setQuestion(nextQuestion); setStage('review'); setBusy('');
    if (home) window.history.replaceState(window.history.state, '', '/request');
  }
  ...
  async function findOptions(event: FormEvent) {
    ...
    try {
      ...
      if (manualReview) { router.push(`/deal/${data.deal.publicToken}`); return; }
  ...
  return <><section id="start-shopping" ref={sectionRef} tabIndex={-1} aria-labelledby="buyer-step-title" className={stage === 'input' ? 'shopping-start' : 'shopping-workspace'}>
    {stage !== 'input' ? <ol className="buyer-progress" aria-label="Buying steps"><li aria-current={stage === 'review' ? 'step' : undefined}><span>1</span> Your request</li><li aria-current={stage === 'options' ? 'step' : undefined}><span>2</span> Compare options</li><li><span>3</span> Review & pay</li></ol> : null}
    {stage === 'input' ? <>
    ...
    {stage === 'review' ? <>
      <header className="shopping-section-heading"><div><span>YOUR REQUEST</span><h1 id="buyer-step-title" ref={headingRef} tabIndex={-1}>A quick check. Then your options.</h1><p>Review what we understood and fill in anything missing.</p></div><button type="button" onClick={() => { setStage('input'); setError(''); }} disabled={Boolean(busy)}><Pencil size={14} /> Rewrite request</button></header>
    ...
    {stage === 'options' ? <>
      ...
      {!options.length ? <div className="shopping-empty"><Package size={34} /><p>{notice}</p><Link href="/catalog">Browse available products →</Link></div> : <div className="shopping-options">{options.map(option => <article className={option.recommended ? 'shopping-option recommended' : 'shopping-option'} key={option.key}>
        <header><span>{option.label}</span>{option.recommended ? <b>{option.recommendationSource === 'mistral' ? 'AI pick' : 'Recommended'}</b> : null}</header>
      ...
      <section className="custom-quote-invite"><div><h2>Need the store’s help?</h2><p>Send this request to The Good Batch for a personal review. To add new requirements, edit your details first.</p></div><button className="subtle-button" type="button" onClick={askStore} disabled={Boolean(selecting)}>{selecting === 'custom' ? 'Sending…' : 'Ask the store for a quote'}</button></section>
    </> : null}
