...
export default async function MerchantDealPage({ params }: { params: Promise<{ dealId: string }> }) {
  ...
  const pending = counteroffers.filter(item => item.status === 'merchant_approval_required' && item.sourceQuoteId === quote?.id && quote.status === 'merchant_approved');
