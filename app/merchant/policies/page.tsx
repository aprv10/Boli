...
export default async function PoliciesPage() {
  ...
  return <main className="new-shell merchant-workspace"><header className="merchant-heading"><div><p className="eyebrow">The Good Batch</p><h1>Rules</h1><p>Set the limits Boli must respect when building and negotiating offers.</p></div></header><section className="merchant-content">
    ...
    <details className="merchant-history"><summary>Advanced · previous rule settings</summary>{history.results.map(item => <p key={item.version}>{item.date.slice(0,10)} · Margin floor {item.margin / 100}% · Automatic reduction limit {item.concession / 100}% <small>Revision {item.version}</small></p>)}</details>
