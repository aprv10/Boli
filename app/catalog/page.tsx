...
export default async function CatalogPage() {
  ...
  return <main className="new-shell"><section className="shopping-workspace">
    ...
    <div className="catalog-grid">{rows.results.map(product => <article key={product.name}><div className="catalog-illustration" aria-hidden="true"><Package size={40} /></div><span>{product.category}</span><h2>{product.name}</h2><strong>₹{(product.price / 100).toLocaleString('en-IN')}</strong><p>{product.stock} available · {product.days} day lead time</p><div className="constraint-ticks">{(JSON.parse(product.tags) as string[]).map(tag => <span key={tag}>{tag}</span>)}</div><Link className="catalog-request-link" href={`/request?product=${encodeURIComponent(product.name)}`}>Request this product →</Link></article>)}</div>
  </section></main>;
}
